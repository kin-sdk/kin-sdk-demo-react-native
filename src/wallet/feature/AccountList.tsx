...
export const AccountList: VFC = () => {
  ...
  const [kin] = useState<KinSDKService>(
    () => new KinSDKService(KinEnvironment.Test),
  );
  ...
  const submitPayment = async (account: KinAccount) => {
    ...
    const destination =
      'GD3RIQOQFRV6SBALVVSR3YGLQHWZA6HZMSODR2ZLKWIRLKFBLQ7AH3EC';
    const amount = '2';
    const memo = 'Test Memo!';

    // Call out to our SDK's submitPayment method
    return kin
      .submitPayment(account.secret, destination, amount, memo)
      .then(result => {
